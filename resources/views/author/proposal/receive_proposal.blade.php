...
@section('title','Received Proposal')
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($receive_proposals as $receive_proposal)
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if($receive_pro_user_image)
                                                        <img src="{{ asset('images/user_profile_image/'. $receive_pro_user_image->image) }}" height="65" width="100">
                                                    @else
                                                        @if($receive_pro_user->gender == 'Female')
                                                ...
                                                <td>
                                                    {{ $receive_pro_user->created_at->format('d-m-y  H:i A') }}
                                                </td>
                                                <td>
                                                    @if($receive_proposal->approved == 0)
                                                ...
                                                <td>
                                                    @if($receive_proposal->approved == 1)
                                                        <a href="{{ route('member.receive-proposal.edit', $receive_proposal->id) }}" class="btn btn-xs btn-success"><i class="fas fa-window-close"></i></a>
                                                    @else
                                                        <a href="{{ route('member.receive-proposal.show', $receive_proposal->id) }}" class="btn btn-xs btn-success"><i class="fa fa-check-circle"></i></a>

                                                        <a href="{{ route('member.receive-proposal.edit', $receive_proposal->id) }}" class="btn btn-xs btn-success"><i class="fas fa-window-close"></i></a>
                                                    @endif
                                                    <form action="{{ route('member.receive-proposal.destroy',$receive_proposal->id ) }}" method="post"
